...
async def test_wrong_token_returns_401(http_client) -> None:
    r = await http_client.get("/api", headers={"Authorization": "Bearer wrong-key"})
    assert r.status_code == 401


async def test_malformed_auth_no_bearer_prefix(http_client) -> None:
    r = await http_client.get("/api", headers={"Authorization": "test-key"})
    assert r.status_code == 401


async def test_empty_token_returns_401(http_client) -> None:
    r = await http_client.get("/api", headers={"Authorization": "Bearer "})
    assert r.status_code == 401


async def test_correct_token_passes_auth(http_client) -> None:
    r = await http_client.get("/api", headers={"Authorization": "Bearer test-key"})
    assert r.status_code != 401
